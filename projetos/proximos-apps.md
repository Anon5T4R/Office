...
## 8. LocalKeys — gerenciador de senhas ⚠️ segurança primeiro — **IMPLEMENTADO (2026-07-15, v0.5.0)**

**Estado:** repo https://github.com/Anon5T4R/LocalKeys (**GPL-3**, público), porta dev **1456**, identifier `com.localkeys.app`, ext `.tkeys`, categoria **Segurança** (nova) no Hub. No catálogo do TaylorHub (release **v0.8.4**) e virou submodule do superprojeto `Local/`. CI verde nas cinco versões (front build + vitest + cargo test + **cargo audit**; desde a v0.4 há **job `check-windows`** que compila/testa o código só-Windows — pegou 2 erros que o Ubuntu não veria); releases NSIS + AppImage publicadas pelo Actions **na 1ª tentativa** (fora um fix de dep na v0.5). **O João rodou o app (v0.3) e confirmou funcionando** — o único furo que ele achou foi o clipboard (resolvido na v0.4).
...
- **v0.5.0:** **auto-type** (crate `enigo`, comando `type_text`; digita usuário/senha no campo em foco sem passar pelo clipboard — o copiar continua; Linux precisa de `libxdo-dev` no CI); **desbloqueio rápido** opt-in (crate `keyring`) que guarda a chave da sessão no cofre do SO — `crypto::open_with_key` decifra sem re-rodar o Argon2; invalida ao trocar a master; **import do export CIFRADO do Bitwarden** (`bwdecrypt.rs`: PBKDF2/Argon2id → HKDF → AES-256-CBC + HMAC-SHA256, só RustCrypto) — **experimental, não testado com arquivo real** (decrypt-only, falha graciosamente).
...
- **Testar na GUI (João):** o import cifrado do Bitwarden com arquivo real (é o único caminho não exercitado — só compila/roundtrip de cripto no CI) e o auto-type/keyring no Windows.
- **Auto-type no Linux/Wayland** é instável (enigo usa xdo/X11) — no Wayland pode não funcionar.
- Menores: auto-lock também imposto pelo back-end; atalho global pra abrir/auto-type; tags além de pastas.
...
8. ~~**LocalKeys**~~ **FEITO (2026-07-15, v0.5.0)** — cofre `.tkeys` (XChaCha20-Poly1305 + Argon2id, chave só no Rust), gerador, auto-lock, TOTP, importadores robustos (Bitwarden claro **e cifrado**/CSV/KeePass), histórico, campos personalizados, anexos cifrados, relatório de fracas/repetidas, gravação atômica, **clipboard fora do histórico do Windows**, zeroize, pastas, **auto-type** e **desbloqueio rápido por keyring**. `SECURITY.md` + `cargo audit` + `check-windows` no CI. **João rodou e aprovou (v0.3).** No Hub (v0.8.4) + submodule. **Falta:** o João testar na GUI o import cifrado do Bitwarden (com arquivo real) e o auto-type/keyring.
